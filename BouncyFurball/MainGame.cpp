...
const Vector2D PADDLE_AABB{ 50.f, 20.f };
...
void StartGame()
{
	...
	Play::CreateGameObject(TYPE_BALL, { (DISPLAY_WIDTH / 2) - 200, DISPLAY_HEIGHT / 2 }, BALL_RADIUS, "Cat_Ball_C");
	Play::CreateGameObject(TYPE_PADDLE, { DISPLAY_WIDTH / 2, DISPLAY_HEIGHT - 100 }, BALL_RADIUS, "laser");
	...
	for (int i = 1; i < 3; i++)
	{
		for (int j = 0; j < 7; j++)
		{
			...
			int id = Play::CreateGameObject(TYPE_CHEST, { gameState.x, gameState.y }, 10, "spr_hum_body");
			GameObject& chestObj{ Play::GetGameObject(id) };
			chestObj.scale = 1.5f;
			gameState.xCount++;
...
void DrawGamePlay()
{
	...
	Play::DrawBackground();
	// Draw the 'paddle'
	Play::DrawObjectRotated(Play::GetGameObjectByType(TYPE_PADDLE));
	...
	{
		Play::DrawObjectRotated(Play::GetGameObject(chest));
	}
...
void ChestCollision()
{
	...
	for (int chest : chestIds)
	{
		...
		if (IsBallColliding(chestObj))
		{
			...
			int random = Play::RandomRoll(7);
			int id = 0;

			if (random == 1)
				id = Play::CreateGameObject(TYPE_COIN, chestObj.pos, 10, "red_m");
			else if (random == 2)
				id = Play::CreateGameObject(TYPE_COIN, chestObj.pos, 10, "pink_m");
			else if (random == 3)
				id = Play::CreateGameObject(TYPE_COIN, chestObj.pos, 10, "ocean_m");
			else if (random == 4)
				id = Play::CreateGameObject(TYPE_COIN, chestObj.pos, 10, "nice_blue_m");
			else if (random == 5)
				id = Play::CreateGameObject(TYPE_COIN, chestObj.pos, 10, "green_m");
			else if (random == 6)
				id = Play::CreateGameObject(TYPE_COIN, chestObj.pos, 10, "yellow_dark_m");
			else if (random == 7)
				id = Play::CreateGameObject(TYPE_COIN, chestObj.pos, 10, "sky_m");


			GameObject& coinObj{ Play::GetGameObject(id) };
			coinObj.scale = 2.0f;
